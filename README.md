...
本地运行默认使用 Node.js 内置 SQLite，数据库和会话密钥会在首次启动时自动创建到 `.local/`，无需预先安装 PostgreSQL。DeepSeek、讯飞语音和蜂鸟地图配置均为可选项；未配置时，系统仍可启动并使用文字交互、确定性业务流程和受限的导诊降级能力。
...
- 患者端：<http://127.0.0.1:3000/user>
- 医生端：<http://127.0.0.1:3000/doctor>
- 管理员端：<http://127.0.0.1:3000/admin>
- 健康检查：<http://127.0.0.1:3000/api/health>

开发时可使用 `npm run dev` 启用文件变更监听。运行 `npm test` 可执行自动化测试。
